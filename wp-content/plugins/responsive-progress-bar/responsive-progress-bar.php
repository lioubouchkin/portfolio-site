	...
	class Responsive_Progressbar {
		...
		function rprogress_shortcode( $atts ) {
			...
			return
				...
				<div class='rprogress-wrap rprogress' data-progress-percent='{$value}' data-speed='{$delay}' style='background: ${color}; border-radius: ${border_radius}; -moz-border-radius: ${border_radius}; -webkit-border-radius: ${border_radius}; -o-border-radius: ${border_radius}; height: 20px;'>
					<div class='rprogress-bar rprogress' style='background: ${bgcolor}'></div><div class='rprogress-text' style='color: ${text_color}; text-align: left; font-family: Lora, Georgia, serif; text-shadow: none; height: 20px; margin-bottom:5px;'><span style='font-size:1rem;position:relative; top:-20%;'>${text}</span></div>
				</div>
				</div>
				";
		}
